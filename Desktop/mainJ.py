...
def turn_to(theta):
    ...
    if pose[2] - delta_theta > 180:
        turn_left(40)
         #print(f"Turning left to {theta}")
    elif pose[2] - theta < 180:
         turn_right(40)  # Adjust speed as necessary for turning, 20 is an example speed
         #print(f"Turning right to {theta}")

def move_to(current_position, target_position, initial_time, t=0):
    move_time = 1 # duration in sconds
    turn_time = 5
    if time.time() - initial_time > move_time:

        if t == 0:
            m_initial_time = time.time()
            t = 1
        ...
        if time.time() - m_initial_time < turn_time:
            if abs(current_position[2] - theta) > 40:
                turn_to(theta)
                print("im turning to")  # Still turning
            elif abs(current_position[2] - theta) < 40:
                ...
                if not at_waypoint:
                    if abs(pose[0] - WAYPOINTS[waypoint][0]) > 0.5 or abs(pose[1] - WAYPOINTS[waypoint][1]) > 0.5:
                        ...
                        if i == 0:
                            current_time = time.time()
                            i += 1
                        move_to(pose, WAYPOINTS[waypoint], current_time)
