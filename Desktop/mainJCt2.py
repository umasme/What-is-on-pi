...
WAYPOINTS = [[0, -2, "mine"], [1.5, -2, "deposit"], [1, -1, "deposit"]]  # Updated waypoints
...
try:
    with contextlib.ExitStack() as stack:
        ...
        WAYPOINTS = [
            ...
            [1, 5, "midpoint"],     # Index 0: Midpoint
            [0, 0, "temporary"],  # Temporary waypoint for initial position
            [2, 4, "mine"],       # Index 1: Excavation zone
            [0, 0, "temporary"],  # Temporary waypoint for initial position
            [1.5, 6, "deposit"]    # Index 2: Deposition zone
        ]
        ...
        def calculate_angle_to_waypoint(current_position, target_position):
        ...
        while True:
            ...
            if need_to_scan:
                ...
                if not scanning_in_progress:
                    ...
                    target_angle = calculate_angle_to_waypoint(pose, WAYPOINTS[current_waypoint_index])
                    print(f"Starting obstacle scan, target angle: {target_angle}, current angle: {pose[2]}")
                    
                # Stage 0: Rotate to face target waypoint
                if scan_stage == 0:
                    print("if scan stage 0")
                    if abs(target_angle - pose[2] + 540) % 360 - 180 > 30:
